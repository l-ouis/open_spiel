...
def _fermi_fixation_probability(u, m):
  """Returns (1 - exp(-u)) / (1 - exp(-m * u)) with overflow guards.

  Args:
    u: Selection intensity times the fitness difference (mutant - resident).
    m: Population size.

  Returns:
    The fixation probability of a single mutant under the Fermi process.
  """
  if np.isclose(u, 0, atol=1e-14):
    return 1. / m
  if u > 0:
    return np.expm1(-u) / np.expm1(-m * u)
  # avoid misleaing overflows when u < 0
  return np.exp((m - 1) * u) * np.expm1(u) / np.expm1(m * u)
...
def _get_rho_sr(payoff_table,
  ...
  if use_local_selection_model or game_is_constant_sum:
    ...
    result = _fermi_fixation_probability(u, m)
  else:
...
def _get_multipop_transition_matrix(payoff_tables,
  ...
  for id_row_profile in range(num_profiles):
    ...
    c[id_row_profile, id_row_profile] = 1 - row_mass
...
def _get_stationary_distr(c, use_sparse=False):
  ...
  if use_sparse:
    ...
    balance = (sps.csr_matrix(c).T - sps.identity(num_states)).tolil()
    balance[-1, :] = 1.
    target = np.zeros(num_states)
    target[-1] = 1.
    with warnings.catch_warnings():
      warnings.simplefilter('ignore', spla.MatrixRankWarning)
      left_vec = spla.spsolve(balance.tocsc(), target)
    if not np.all(np.isfinite(left_vec)) or np.min(left_vec) < -1e-8:
      raise ValueError('Expected 1 stationary distribution, but found more')
  else:
    if sps.issparse(c):
      c = c.toarray()
...
def print_results(payoff_tables,
  ...
  if c is not None:
    c_to_print = c.toarray() if sps.issparse(c) else c
